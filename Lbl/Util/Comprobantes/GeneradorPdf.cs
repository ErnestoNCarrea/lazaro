...
#pragma warning restore CS3003 // Il tipo non è conforme a CLS
        ...
        /// <summary>
        /// Genera el texto que va en el QR de la factura, según AFIP.
        /// https://www.afip.gob.ar/fe/qr/especificaciones.asp
        /// </summary>
        public string GenerarTextoQR()
        {
            var qrUrl = "https://www.afip.gob.ar/fe/qr/?p=";
            var qrDatos = new Dictionary<string, string>();
            qrDatos.Add("ver", "1");
            qrDatos.Add("fecha", "\"" + this.Comprob.Fecha.ToString("yyyy/MM/dd") + "\"");
            qrDatos.Add("cuit", Lbl.Sys.Config.Empresa.ClaveTributaria.ToString().Replace("-", ""));
            qrDatos.Add("ptoVta", Comprob.PV.ToString());
            qrDatos.Add("tipoCmp", Lbl.Comprobantes.Comprobante.TipoCompAfip(Comprob.Tipo.Nomenclatura).ToString());
            qrDatos.Add("nroCmp", Comprob.Numero.ToString());
            qrDatos.Add("importe", Lfx.Types.Formatting.FormatCurrencySql(Comprob.Total));
            qrDatos.Add("moneda", "\"PES\"");
            qrDatos.Add("ctz", "1");
            if(Comprob.Cliente.ClaveTributaria != null) {
                qrDatos.Add("tipoDocRec", "80");    // CUIT
                qrDatos.Add("nroDocRec", Comprob.Cliente.ClaveTributaria.ToString().Replace("-", ""));
            } else {
                qrDatos.Add("tipoDocRec", "96");    // DNI
                qrDatos.Add("nroDocRec", Comprob.Cliente.NumeroDocumento.Replace(".", ""));
            }
            qrDatos.Add("tipoCodAut", "\"E\"");
            qrDatos.Add("codAut", Comprob.CaeNumero.ToString());
            ...
            return qrUrl + "{" + string.Join(",", qrDatos.Select(x => x.Key + "=" + x.Value)) + "}";
        }
        ...
        public PdfDocument GenerarFactura()
        {
            ...
            Gfx.DrawImage(XImagenCodBarras, new XRect(CuadroPie.Left + 28 * mm, CuadroPie.Bottom - 20 * mm, 90 * mm, 12 * mm));
            Tf.Alignment = XParagraphAlignment.Center;
            Tf.DrawString(TextoCodigoBarras,
                FuentePredeterminada,
                XBrushes.Black,
                new XRect(CuadroPie.Left + 28 * mm, CuadroPie.Bottom - 7 * mm, 90 * mm, 7 * mm)
            );

            string QrTexto = this.GenerarTextoQR();
            var QrData = QRCodeGenerator.GenerateQrCode(QrTexto, QRCodeGenerator.ECCLevel.L);
            var BitmapQr = new QRCode(QrData).GetGraphic(20);

            Gfx.DrawImage(BitmapQr, new XRect(CuadroPie.Left, CuadroPie.Bottom - 30 * mm, 30 * mm, 30 * mm));
